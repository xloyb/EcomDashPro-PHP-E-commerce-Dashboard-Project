            ...
            <div class="mt-6 grid grid-cols-1 gap-y-10 sm:grid-cols-3 sm:gap-x-6 sm:gap-y-0 lg:gap-x-8">
                ...
                ?>
                    <div data-category-id="<?php echo htmlspecialchars($row['CategoryID']); ?>" class="louayprod product group relative" data-category-id="<?php echo htmlspecialchars($row['CategoryID']); ?>">
                        ...
                        <p class="mt-1 text-sm text-gray-500">Stock Quantity: <?php echo $row['StockQuantity']; ?></p>
                            <!-- Form to add the article to the buying list -->
                            <form method="post" action="user_dashboard.php" class="d-flex">
                                <input type="hidden" name="article_id" value="<?php echo $row['ArticleID']; ?>">
                                <div class="input-group">
                                    <label for="quantity" class="input-group-text">Quantity:</label>
                                    <input type="number" name="quantity" value="1" min="1" class="form-control" style="width: 80px;">
                                    <button type="submit" name="add_to_buying_list" style="background: #6e90de;" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Add to Buying List</button>
                                </div>
                            </form>
                    </div>
                <?php
